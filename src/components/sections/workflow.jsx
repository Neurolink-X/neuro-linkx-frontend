...
export default function Workflow() {
  ...
  <motion.div
    ...
    className='relative mb-12 lg:mb-0' // <-- added this line
  >
    <div className='flex flex-col h-full min-h-[200px] p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-[#ff3b30]/20 transition-all duration-300 shadow-lg shadow-[#ff3b30]/5'>
      <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${step.color} flex items-center justify-center mb-4`}>
        <step.icon className='w-6 h-6 text-white' />
      </div>
      <h3 className='text-2xl font-bold text-white mb-2'>{step.title}</h3>
      <p className='text-white/70 flex-grow'>{step.description}</p>
    </div>
